...
/**
 * The following 4 methods are meant in case a custom gesture recognizer is implemented outside the view that doesn't conform
 * to the UIGestureRecognizerDelegate protocol.
 *
 * Before these methods are used, please make sure to overwrite the gesture recognizers on this view by setting them to nil
 * so that they won't interfere with the custom touch events.
 *
 * The touch parameter should uniquely identify a certain touch through its lifetime (begin -> moved -> ended or cancelled).
 * The x and y parameters should provide the location of the touch event in screen coordinates, taking into account device orientation changes.
 */
- (void)onTouchBegan:(int)touch withX:(double)x andY:(double)y;
- (void)onTouchEnded:(int)touch withX:(double)x andY:(double)y;
- (void)onTouchCancelled:(int)touch withX:(double)x andY:(double)y;
- (void)onTouchMoved:(int)touch withX:(double)x andY:(double)y;
