...
namespace wikitude { namespace sdk {
    ...
    struct WT_EXPORT_API Rectangle {
        ...
        Rectangle(Point<T> origin_, Size<T> size_)
            : origin(origin_)
            , size(size_) {
        }
    ...
    struct WT_EXPORT_API Scale2D {
        Scale2D()
            : x(0)
            , y(0) { /* Intentionally Left Blank */
        }

        Scale2D(T x_, T y_)
            : x(x_)
            , y(y_) { /* Intentionally Left Blank */
        }
    ...
    struct WT_EXPORT_API Scale3D {
        Scale3D()
            : x(0)
            , y(0)
            , z(0) { /* Intentionally Left Blank */
        }

        Scale3D(T x_, T y_, T z_)
            : x(x_)
            , y(y_)
            , z(z_) { /* Intentionally Left Blank */
        }
    ...
    struct WT_EXPORT_API Extent {
        Extent()
            : min(0)
            , max(0) { /* Intentionally Left Blank */
        }

        Extent(T min_, T max_)
            : min(min_)
            , max(max_) { /* Intentionally Left Blank */
        }

        Extent(const Extent& extent_)
            : min(extent_.min)
            , max(extent_.max) { /* Intentionally Left Blank */
        }

        Extent& operator=(const Extent& extent_) {
            if (this != &extent_) {
                min = extent_.min;
                max = extent_.max;
            }
