...
typedef NS_ENUM(NSUInteger, WTFlutterMethodCall) {
    ...
    WTFlutterMethodCall_OnTouchBegan,
    WTFlutterMethodCall_OnTouchEnded,
    WTFlutterMethodCall_OnTouchCancelled,
    WTFlutterMethodCall_OnTouchMoved,
};
...
- (instancetype)initWithFrame:(CGRect)frame viewIdentifier:(int64_t)viewId arguments:(id)args registrar:(NSObject<FlutterPluginRegistrar> *)registrar
{
    if ([super init])
    {
        ...
        _flutterMethodCallsArray = @[@"load",@"onPause", @"onResume", @"onDestroy", @"setLocation", @"callJavascript",
                                     @"addArchitectJavaScriptInterfaceListener", @"captureScreen", @"showAlert", @"onTouchBegan", @"onTouchEnded", @"onTouchCancelled", @"onTouchMoved"];
        ...
        /* Because the gestures will be overwritten from Flutter, make sure to disable any existing gesture recognizers on the ArchitectView. */
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result
{
    ...
    switch (methodCallIndex) {
        ...
        case WTFlutterMethodCall_ShowAlert:
            ...
            break;
        case WTFlutterMethodCall_OnTouchBegan:
            [self onTouchBegan:call result:result];
            break;
        case WTFlutterMethodCall_OnTouchEnded:
            [self onTouchEnded:call result:result];
            break;
        case WTFlutterMethodCall_OnTouchCancelled:
            [self onTouchCancelled:call result:result];
            break;
        case WTFlutterMethodCall_OnTouchMoved:
            [self onTouchMoved:call result:result];
            break;
        default:
...
TouchConversionResult convertFlutterTouch(FlutterMethodCall* call) {
    TouchConversionResult result;
    result.success = false;
    result.pointer = 0;
    result.x = 0;
    result.y = 0;

    if ([call.arguments[@"pointer"] isKindOfClass:[NSNumber class]]) {
        result.pointer = [(NSNumber*)call.arguments[@"pointer"] intValue];
    } else {
        NSLog(@"Invalid Flutter touch pointer ID for method %@", call.method);
        return result;
    }

    if ([call.arguments[@"x"] isKindOfClass:[NSNumber class]]) {
        result.x = [(NSNumber*)call.arguments[@"x"] doubleValue];
    } else {
        NSLog(@"Invalid Flutter touch X coordinate for method %@", call.method);
        return result;
    }

    if ([call.arguments[@"y"] isKindOfClass:[NSNumber class]]) {
        result.y = [(NSNumber*)call.arguments[@"y"] doubleValue];
    } else {
        NSLog(@"Invalid Flutter touch Y coordinate for method %@", call.method);
        return result;
    }
...
- (void)onTouchBegan:(FlutterMethodCall*)call result:(FlutterResult)result
{
    TouchConversionResult convertedTouch = convertFlutterTouch(call);
    if (convertedTouch.success) {
        [self.architectView onTouchBegan:convertedTouch.pointer withX:convertedTouch.x andY:convertedTouch.y];
    }
}

- (void)onTouchEnded:(FlutterMethodCall*)call result:(FlutterResult)result
{
    TouchConversionResult convertedTouch = convertFlutterTouch(call);
    if (convertedTouch.success) {
        [self.architectView onTouchEnded:convertedTouch.pointer withX:convertedTouch.x andY:convertedTouch.y];
    }
}

- (void)onTouchCancelled:(FlutterMethodCall*)call result:(FlutterResult)result
{
    TouchConversionResult convertedTouch = convertFlutterTouch(call);
    if (convertedTouch.success) {
        [self.architectView onTouchCancelled:convertedTouch.pointer withX:convertedTouch.x andY:convertedTouch.y];
    }
}

- (void)onTouchMoved:(FlutterMethodCall*)call result:(FlutterResult)result
{
    TouchConversionResult convertedTouch = convertFlutterTouch(call);
    if (convertedTouch.success) {
        [self.architectView onTouchMoved:convertedTouch.pointer withX:convertedTouch.x andY:convertedTouch.y];
    }
}
