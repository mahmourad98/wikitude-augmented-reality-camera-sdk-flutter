...
public class ArchitectWidget implements PlatformView, MethodCallHandler, ArchitectView.ArchitectWorldLoadedListener, ArchitectJavaScriptInterfaceListener, LocationListener {
    ...
    ArchitectWidget(@NonNull BinaryMessenger messenger, FlutterPlugin.FlutterAssets flutterAssets, int id, Object o) {
        this.binaryMessenger = messenger;
        this.flutterAssets = flutterAssets;
        this.id = id;

        gson = new Gson();
        this.startUpConfiguration = gson.toJson(o);

        WebView.setWebContentsDebuggingEnabled(true);

        channel = new MethodChannel(binaryMessenger, "architectwidget_" + id);
    ...
    public void setContext(Context context) {
        ...
        try {
            JSONObject jsonObject = new JSONObject(startUpConfiguration);
        ...
        switch (call.method) {
            case "load":
                ...
                if(!url.contains("https://") && !url.contains("http://") && !url.startsWith("file://")
                    ...
                    url = flutterAssets.getAssetFilePathBySubpath(url);
